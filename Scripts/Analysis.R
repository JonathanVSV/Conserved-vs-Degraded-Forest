...
df_long <- df %>%
  ...
  select(Sitio, tipo_bosque,Cobertura, AB_m2.ha, Media_altura, biomasa_mg.ha, Media_altura, Total_ramas, árboles.ha) %>%
  # Prepare data: long format. Indicate which columns are going into long format
  pivot_longer(cols = c(Cobertura, AB_m2.ha, Media_altura, biomasa_mg.ha, Media_altura, Total_ramas, árboles.ha),
               names_to = "Attribute",
               values_to = "value")

...
df <- df %>%
  ...
  pivot_longer(cols = c(Cobertura, AB_m2.ha, Media_altura, biomasa_mg.ha),
               names_to = "Attribute",
               values_to = "value") %>%
  group_by(tipo_bosque, Attribute) %>%
...
p1 <- df  %>%
  ...
  # Add errorbar
  geom_errorbar(aes(ymin=mean_val-2*se_val, 
                    ymax=mean_val+2*se_val, 
                    width=.2)) +
  # Add threshold line
  geom_hline(yintercept = 8.02, col = "red", lty = "longdash")+
  # Add text for the threshold's value
  geom_text(aes(x = 1.55, y = 8.55, label = "8.02")) +
...
p2 <- df  %>%
  ...
  geom_text(aes(x = 1.55, y = 24.7, label = "23.3"))+
...
p3 <- df  %>%
  ...
  geom_text(aes(x = 1.55, y = 93.7, label = "90.2"))+
...
## Save joined plot
save_plot("Final_barplot.jpg", p_final, 
          base_height = 8,
          base_asp = 0.85)
